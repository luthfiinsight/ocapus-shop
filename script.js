document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyFilters() {
        ...
        const selectedCategory = categoryFilter.value; // Nilai ini sudah lowercase
        const selectedEcommerce = ecommerceFilter.value;

        filteredProducts = allProducts.filter(product => {
            const matchesSearch = product.judul.toLowerCase().includes(searchTerm);
            
            // --- BAGIAN INI DIPERBAIKI UNTUK MENGATASI ERROR PADA KATEGORI KOSONG ---
            // Cek dulu apakah product.kategori ada isinya sebelum di proses
    ...
    // --- FUNGSI PEMBUATAN FILTER KATEGORI (DIPERBARUI) ---
    function setupCategoryFilter() {
        ...
        uniqueCategories.sort().forEach(category => {
            // Tampilan tetap menggunakan format asli, tapi value dibuat lowercase untuk konsistensi filter
